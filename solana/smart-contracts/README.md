...
```bash
solana program deploy dist/program/documents.so
```
...
### Check if the documents on-chain program has been deployed
...
the client loads the keypair of the deployed program from `./dist/program/documents-keypair.json` and uses
